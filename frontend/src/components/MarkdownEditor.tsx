  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-xs text-gray-600 ml-auto">Markdown</span>
      </div>
      {preview ? (
        <div className={`bg-gray-800 border border-gray-700 rounded px-3 py-2 ${className}`} style={{ minHeight: `${rows * 1.5}rem` }}>
          {value.trim() ? (
            ...
            <p className="text-sm text-gray-600 italic">Keine Vorschau</p>
          )}
        </div>
      ) : (
        <textarea
          ...
          className={`w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-blue-500 resize-none font-mono ${className}`}
        />
      )}
    </div>
  );
